...
const beforeReq = (config) => {
  ...
  // ✅ FormData를 사용할 때는 Content-Type을 제거 (axios가 자동으로 설정)
  // 수동으로 설정하면 boundary 파라미터가 없어서 서버가 파싱하지 못하고,
  // Authorization 헤더도 덮어씌워질 수 있습니다.
  if (config.data instanceof FormData) {
    delete config.headers["Content-Type"];
    console.log("[REQ] FormData detected - Content-Type removed");
  }
...
const beforeRes = async (res) => {
  ...
  if (data && (data.error === "ERROR_ACCESS_TOKEN" || data.message === "ERROR_ACCESS_TOKEN")) {
    ...
    if (res.config._retry) {
      console.log("[JWT] 이미 재시도함 (beforeRes) - 더 이상 재시도 안 함");
      return res;
    }
    ...
    try {
      ...
      // 원래 요청 재시도
      const originalRequest = { ...res.config }; // ✅ config 복사
...
const responseFail = async (err) => {
  ...
  if (err.config?._retry) {
    console.log("[JWT] 이미 재시도한 요청 - 더 이상 재시도하지 않음");
    return Promise.reject(err);
  }

  // ✅ multipart/form-data 요청은 재시도하지 않음 (FormData는 한 번만 전송 가능)
  const contentType = err.config?.headers?.["Content-Type"] || "";
  const isMultipart = contentType.includes("multipart/form-data") || 
                      err.config?.data instanceof FormData;
  
  if (isMultipart) {
    console.log("[JWT] multipart/form-data 요청은 재시도 불가 - 에러 반환");
    return Promise.reject(err);
  }
  ...
  if (status === 401 && (errorData?.message === "ERROR_ACCESS_TOKEN" || errorData?.error === "ERROR_ACCESS_TOKEN")) {
    ...
    try {
      ...
      // ✅ 원래 요청 재시도 (config 복사 후 수정)
      const originalRequest = { ...err.config }; // config 복사
      originalRequest._retry = true; // 재시도 플래그 설정
      originalRequest.headers = {
        ...originalRequest.headers,
        Authorization: `Bearer ${result.accessToken}`,

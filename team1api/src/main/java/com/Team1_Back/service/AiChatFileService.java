...
import com.Team1_Back.repository.ChatMessageRepository; // ✅ 추가
...
public class AiChatFileService {
    ...
    public AiChatFileResponse findChatFilesGlobal(String query) {
        ...
        if (query == null || query.trim().isEmpty()) {
            return new AiChatFileResponse("query is required", List.of());
        }

        String likeQ = toLikePattern(query);

        log.info("[AI-FILE] query='{}' likeQ='{}'", query, likeQ);

        List<ChatAttachmentSearchRow> candidates =
                chatAttachmentSearchService.searchGlobal(me, likeQ, 30, 0);

        log.info("[AI-FILE] candidates.size={}", candidates.size());

        if (candidates.isEmpty()) {
            return new AiChatFileResponse("관련된 파일을 찾지 못했습니다.", List.of());
        }
    ...
    public AiChatFileResponse findFilesByContext(String userQuery, List<Long> messageIds) {
        ...
        // =========================================================
        // 1️⃣ 메시지에 직접 달린 파일 후보 (findByMessage_Id 유지 전제)
        // =========================================================
        List<AiChatFileItem> directCandidates =
                attachmentQueryService.findDirectAttachmentsByMessages(messageIds);

        log.info("[AI-FILE] context messageIds={} directCandidates={}",
                messageIds.size(), directCandidates.size());
        ...
        //   - ChatMessage.createdAt은 Instant
        //   - ChatAttachment.createdAt은 LocalDateTime (Repo/DB 유지)
        //   => 여기(Service)에서만 Instant -> LocalDateTime 변환
        // =========================================================
        ...
        // messageIds가 여러 room에 섞일 수도 있으니 room별로 처리
        Map<Long, List<MessageTimeView>> byRoom = times.stream()
                .filter(t -> t.roomId() != null && t.createdAt() != null)
        ...
        log.info("[AI-FILE] fallbackCandidates={} (nearby ±{}m)", fallbackCandidates.size(), nearbyMinutes);
    ...
    private AiChatFileResponse pickBestFilesFromSearchRows(String userQuery, List<ChatAttachmentSearchRow> candidates) {
        ...
        String prompt = buildPromptForFiles(candidates, userQuery);
        ...
        Map<Long, ChatAttachmentSearchRow> map = candidates.stream()
    ...
    private AiChatFileResponse pickBestFilesFromItems(String userQuery, List<AiChatFileItem> candidates) {
        ...
        String prompt = buildPromptForFileItems(candidates, userQuery);
        ...
        Map<Long, AiChatFileItem> map = candidates.stream()
